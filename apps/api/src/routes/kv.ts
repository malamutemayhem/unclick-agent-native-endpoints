...
function orgKey(orgId: string, key: string) {
  return and(eq(kvStore.orgId, orgId), eq(kvStore.key, key))!;
}
...
export function createKvRouter(db: Db) {
  ...
  router.post('/get', requireScope('kv:read'), zv('json', KeySchema), async (c) => {
    ...
    const [row] = await db
      .select()
      .from(kvStore)
      .where(and(orgKey(orgId, key), notExpired()))
      .limit(1);

    if (!row) throw Errors.notFound(`Key "${key}" not found`);
  ...
  router.post('/delete', requireScope('kv:write'), zv('json', KeySchema), async (c) => {
    ...
    await db.delete(kvStore).where(orgKey(orgId, key));
  ...
  router.post('/list', requireScope('kv:read'), zv('json', ListSchema), async (c) => {
    ...
    const baseWhere = and(eq(kvStore.orgId, orgId), notExpired())!;
    // Prefix filter uses LIKE (prefix + '%')
    const where = prefix
      ? and(baseWhere, sql`${kvStore.key} LIKE ${prefix + '%'}`)!
      : baseWhere;
  ...
  router.post('/increment', requireScope('kv:write'), zv('json', IncrementSchema), async (c) => {
    ...
    if (!row) {
      ...
      return ok(c, { key, value: newValue });
    }
    ...
    try {
      ...
      throw Errors.validation(`Value for key "${key}" is not valid JSON`);
    }

    if (typeof current !== 'number') {
      throw Errors.validation(`Cannot increment: value for key "${key}" is not a number`);
    }
