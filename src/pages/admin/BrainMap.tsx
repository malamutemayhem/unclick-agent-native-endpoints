...
export default function BrainMap() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const load = async (token: string) => {
        const [bootJson, ctxJson, factsJson, sessJson] = await Promise.all([
        ...
        setBootSummary(bootJson.data ?? bootJson ?? null);
        setContextRows(ctxJson.data ?? []);
        setFactCount((factsJson.data ?? []).length);
  ...
  const checklist = [
    { ok: hasIdentity, label: "Identity set", link: "/admin/memory/identity" },
    { ok: hasPreference, label: "Preferences set", link: "/admin/memory/identity" },
    { ok: hasFiveFacts, label: "At least 5 facts", link: "/admin/memory/knowledge" },
    { ok: hasSession, label: "A saved session", link: "/admin/memory/sessions" },
    { ok: hasStandingRule, label: "Standing rules", link: "/admin/memory/identity" },
    { ok: hasRepo, label: "Repository context", link: "/admin/memory/codebase" },
  ];
  ...
  const stages = [
    ...
    { num: "3", title: "Identity", sub: "Who are you?", desc: `Your brand, preferences, and standing rules. ${identityCount} items stored.`, color: "#E2B93B", icon: Fingerprint, link: "/admin/memory/identity" },
    { num: "4", title: "Project Scope", sub: "What are we working on?", desc: "If a project is active, project-scoped memory loads alongside global. Project wins on conflicts.", color: "#61C1C4", icon: FolderKanban, link: "/admin/memory/projects" },
    { num: "5", title: "Codebase", sub: "How is the code shaped?", desc: `Tech stack, deploy process, constraints, gotchas. ${repoCount} items stored.`, color: "#4ade80", icon: Code, link: "/admin/memory/codebase" },
    { num: "6", title: "Knowledge", sub: "What do I know?", desc: `Facts extracted from conversations. ${factCount} items stored.`, color: "#f472b6", icon: Lightbulb, link: "/admin/memory/knowledge" },
    { num: "7", title: "Sessions", sub: "What happened recently?", desc: "Summaries of past conversations. Last 5 load at startup.", color: "#fb923c", icon: Clock, link: "/admin/memory/sessions" },
  ...
  if (!session && !localStorage.getItem("unclick_api_key")) {
    return (
      <div className="rounded-xl border border-white/[0.06] bg-[#111] p-8 text-center">
        <p className="text-sm text-white/70">Sign in to view your brain map.</p>
      </div>
    );
  }
        ...
        {stages.map((stage, idx) => {
          ...
          return (
            <div key={stage.num}>
              {phase && (
                <div
                  className={`mb-2 ml-12 text-[10px] font-bold uppercase tracking-widest ${phase.color}`}
                >
                  {phase.text}
                </div>
              )}
