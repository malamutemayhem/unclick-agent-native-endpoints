...
import {
  ...
  Send, MessagesSquare, Gamepad2, Users, Cloud, Radio,
  ShoppingBag, Package, Receipt, Key, TrendingUp,
} from "lucide-react";
...
const tools: Tool[] = [
  ...
  {
    name: "Telegram",
    description: "Send messages, media, and manage chats through the Telegram Bot API without SDK setup. Supports text with Markdown or HTML formatting, photos, documents, audio, and video. Built for agents that deliver alerts, post reports to groups or channels, and monitor incoming messages as part of automated workflows.",
    endpoint: "/v1/telegram",
    category: "Platform",
    Icon: Send,
    capabilities: [
      "Send text messages with Markdown or HTML formatting to any chat or channel",
      "Send photos, documents, audio, and video files by URL or file ID",
      "Read recent messages and search by keyword across chat history",
      "Manage chats: get info, list members, pin and unpin messages",
    ],
    examplePrompt: "Ask your AI to send a daily digest to your Telegram group every morning",
  },
  {
    name: "Slack",
    description: "Post messages, read channels, reply to threads, and upload files to Slack workspaces via a unified API — no SDK or complex OAuth required. Built for agents that report workflow results, monitor team conversations, participate in approval threads, and automate notification pipelines inside Slack.",
    endpoint: "/v1/slack",
    category: "Platform",
    Icon: MessagesSquare,
    capabilities: [
      "Send messages to any channel or DM with full Slack Block Kit support",
      "Read recent messages and search conversations by keyword",
      "Reply to threads, add emoji reactions, and upload files",
      "List available channels and fetch channel history",
    ],
    examplePrompt: "Ask your AI to post a deployment summary to your #engineering Slack channel",
  },
  {
    name: "Discord",
    description: "Send messages, manage threads, react to posts, and inspect guild members through the Discord Bot API. Supports channel reads, thread creation, message search, and server browsing. Built for agents running community bots, automating game server notifications, and orchestrating Discord-based approval workflows.",
    endpoint: "/v1/discord",
    category: "Platform",
    Icon: Gamepad2,
    capabilities: [
      "Send messages and rich embeds to any guild channel",
      "Create threads and reply to existing thread conversations",
      "React to messages with emoji and search message history in a guild",
      "List guild channels and members with role filtering",
    ],
    examplePrompt: "Ask your AI to post a release announcement to your Discord #announcements channel",
  },
  {
    name: "Reddit",
    description: "Read posts, submit content, comment, search, vote, and manage subreddit subscriptions through the Reddit API. Supports all four feed types (hot, new, top, rising), user profile lookups, and comment threading. Built for agents that monitor communities, surface trending topics, and participate in Reddit-based research workflows.",
    endpoint: "/v1/reddit",
    category: "Platform",
    Icon: Users,
    capabilities: [
      "Read subreddit feeds: hot, new, top, and rising post listings",
      "Submit text or link posts and comment or reply on any thread",
      "Search posts across Reddit with full query and sort support",
      "Upvote/downvote posts and comments; subscribe or unsubscribe from subreddits",
    ],
    examplePrompt: "Ask your AI to find the top 10 posts in r/MachineLearning from the past week",
  },
  {
    name: "Bluesky",
    description: "Post, reply, search, like, repost, and follow on Bluesky using the AT Protocol without managing DIDs or lexicons directly. Built for agents publishing content to the decentralized social graph, monitoring topic feeds, syncing cross-platform posts, and growing audiences on Bluesky.",
    endpoint: "/v1/bluesky",
    category: "Platform",
    Icon: Cloud,
    capabilities: [
      "Post toots and threads with rich text and link card previews",
      "Reply, like, and repost to engage with existing content",
      "Search posts and profiles across the Bluesky network",
      "Follow and unfollow accounts; fetch user profiles and follower counts",
    ],
    examplePrompt: "Ask your AI to cross-post your latest blog article to Bluesky with a link preview",
  },
  {
    name: "Mastodon",
    description: "Post toots, read timelines, reply, boost, favorite, and follow on any Mastodon instance via the Fediverse API. Supports content warnings, visibility settings (public, unlisted, followers-only, direct), and hashtag or account search. Built for agents running decentralized social publishing workflows.",
    endpoint: "/v1/mastodon",
    category: "Platform",
    Icon: Radio,
    capabilities: [
      "Post toots with visibility controls, content warnings, and media attachments",
      "Read home, local, and public timelines from any Mastodon instance",
      "Reply to statuses, boost, and favorite posts",
      "Search posts, accounts, and hashtags; follow or unfollow accounts",
    ],
    examplePrompt: "Ask your AI to post an announcement toot to your Mastodon account with a content warning",
  },
  // ── E-commerce ──────────────────────────────────────────────────────────────
  {
    name: "Shopify",
    description: "Query and manage products, orders, customers, inventory, collections, and fulfillments through the Shopify Admin REST API. Built for agents running store operations, syncing catalog data, automating order management, and building headless commerce workflows without logging into the Shopify dashboard.",
    endpoint: "/v1/shopify",
    category: "Platform",
    Icon: ShoppingBag,
    capabilities: [
      "List, get, create, and update products and variants",
      "Retrieve and filter orders with status, date, and customer filters",
      "Search customers and manage customer records",
      "Track inventory levels by item and location; manage fulfillments",
    ],
    examplePrompt: "Ask your AI to list all unfulfilled orders from the last 7 days and flag high-value ones",
  },
  {
    name: "Amazon",
    description: "Search products, retrieve detailed listings, browse categories, and inspect product variations using the Amazon Product Advertising API 5.0. Built for agents building product comparison tools, affiliate content workflows, price monitoring systems, and e-commerce research pipelines without scraping.",
    endpoint: "/v1/amazon",
    category: "Platform",
    Icon: Package,
    capabilities: [
      "Search Amazon products by keyword, category, or browse node",
      "Get detailed product info by ASIN: price, rating, availability, and images",
      "Browse product categories and subcategories programmatically",
      "Retrieve product variations (size, color, style) for any parent ASIN",
    ],
    examplePrompt: "Ask your AI to find the top-rated noise-cancelling headphones on Amazon under $150",
  },
  // ── Finance ─────────────────────────────────────────────────────────────────
  {
    name: "Xero",
    description: "Read and manage invoices, contacts, accounts, payments, bank transactions, and financial reports through the Xero Accounting API. Built for agents automating bookkeeping workflows, generating P&L summaries, recording payments, and syncing billing data between Xero and other business systems.",
    endpoint: "/v1/xero",
    category: "Platform",
    Icon: Receipt,
    capabilities: [
      "List, retrieve, and create invoices; record payments against them",
      "Manage contacts: customers and suppliers with full details",
      "Retrieve chart of accounts and bank transaction history",
      "Pull financial reports: P&L, Balance Sheet, and Trial Balance",
    ],
    examplePrompt: "Ask your AI to pull this month's P&L report from Xero and summarize the key figures",
  },
  // ── Security & Intelligence ─────────────────────────────────────────────────
  {
    name: "Vault",
    description: "Store, retrieve, and rotate sensitive credentials with AES-256-GCM encryption locally on your machine. No third-party service required — credentials never leave your environment. Built for agents that need to securely manage API keys, tokens, and passwords across multi-step workflows without hardcoding secrets.",
    endpoint: "/v1/vault",
    category: "Security",
    Icon: Key,
    capabilities: [
      "Store and retrieve secrets with AES-256-GCM local encryption",
      "List stored credential keys without exposing their values",
      "Rotate credentials with automatic re-encryption and audit logging",
      "Delete secrets and review a full tamper-evident audit trail",
    ],
    examplePrompt: "Ask your AI to securely store your API keys in the vault and retrieve them only when needed",
  },
  {
    name: "C-Suite Analyze",
    description: "Run any business scenario, decision, or strategy document through twelve C-suite executive perspectives simultaneously — CEO, COO, CTO, CFO, CMO, CIO, CHRO, CDO, CPO, CSO, CCO, and CAIO. Returns structured analysis at quick, standard, or deep depth. Built for agents supporting strategic planning, board prep, and executive decision-making workflows.",
    endpoint: "/v1/csuite",
    category: "Platform",
    Icon: TrendingUp,
    capabilities: [
      "Analyze decisions through 12 C-suite lenses: CEO, CTO, CFO, CMO, and more",
      "Three depth modes: quick (key points), standard (full analysis), deep (comprehensive)",
      "Structured output per executive: risks, opportunities, and recommendations",
      "Identify cross-functional conflicts and alignment gaps across perspectives",
    ],
    examplePrompt: "Ask your AI to analyze a proposed product pivot through all 12 executive perspectives",
  },
];
